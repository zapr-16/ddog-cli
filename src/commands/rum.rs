...
///   ddog rum --query "@view.name:*checkout*" --limit 50
#[derive(Args)]
pub struct RumSearch {
    /// RUM query using Datadog syntax (e.g., "@type:action", "@application.name:myapp")
...
pub async fn search(client: &DdClient, args: RumSearch) -> Result<(), DdError> {
    let range = time::resolve_range(&args.from, &args.to, limits::MAX_RUM_HOURS)
        .map_err(DdError::Validation)?;
    let limit =
        limits::resolve_limit(args.limit, limits::MAX_SEARCH_LIMIT).map_err(DdError::Validation)?;

    log::info(&format!(
        "Searching RUM events: query=\"{}\" range={} limit={limit}",
        args.query,
    ...
    let result = client.post("/api/v2/rum/events/search", &body).await?;
    let count = print_output(&result, &args.format, RUM_COLUMNS);
    log::result_count(count, "RUM events");
    Ok(())
}
...
mod tests {
    ...
    fn test_parse_custom() {
        ...
        assert_eq!(cli.args.query, "@type:action");
        assert_eq!(cli.args.from, "30m");
